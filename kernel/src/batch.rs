...
impl AppManagerInner {
    ...
    unsafe fn load_app(&self, app_id: usize)->usize {
        ...
        // get address
        let laddr = unsafe{ *(app_src.as_ptr() as *const usize) };
        // clear app area
        (laddr..laddr + APP_SIZE_LIMIT).for_each(|addr| {
...
pub fn run_next_app() -> ! {
    ...
    let mut eaddr = 0x0;
    unsafe {
        eaddr = APP_MANAGER.inner.borrow().load_app(current_app);
    }
    APP_MANAGER.inner.borrow_mut().move_to_next_app();
    extern "C" { fn __restore(cx_addr: usize); }
    unsafe {
        __restore(KERNEL_STACK.push_context(
            TrapContext::app_init_context(eaddr, USER_STACK.get_sp())
        ) as *const _ as usize);
    }
